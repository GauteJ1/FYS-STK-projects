...
We denote the inner domain as $\Omega = (0, L) \times (0, T)$, with the initial boundary $\partial\Omega_I = (0, L) \times \{0\}$ and side boundaries $\partial\Omega_0 =  \{0\} \times [0, T]$ and $\partial\Omega_L = \{L\} \times [0, T]$.
Furthermore, we denote by $N$ the number of points $(x, t) \in \Omega$, by $N_I$ the number of points in the initial condition area $(x,t) \in \Omega_I$ and finally by $N_0$ and $N_L$ the number of points on the left and right side boundary $(x,t) \in \Omega_0$ and $(x, t) \in \Omega_L$ respectively.
...
Lastly, the boundary values should be zero as given in Eq. \ref{eq:boundaries_heat_eq}. 
