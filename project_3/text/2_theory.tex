...
\julie{could add examples of PDE's, left for potential fluff later}

\julie{assuming homogeneous/nonhomogeneous and linear/nonlinear is not of relevance, thus left out}
...
Boundary conditions are similar to initial conditions, but in stead of describing a state at some starting point the condition now provides the state of the system at some boundary point. These boundary points commonly describe points at the edges of a functions domain. For a PDE of a function $u(x,t)$ on the domain $[0, L]$, the boundary conditions might be stated
\begin{equation}
    \begin{split}
...
\end{equation}
By making a slight assumption about the form of the solution and using the initial conditions, one can derive the analytical solution given in Eq. \ref{analytical_sol}. By insertion in the PDE and it's condition it is easily verified. The full derivation as well as verification of the solution can be found in Appendix \ref{appendixB}.
