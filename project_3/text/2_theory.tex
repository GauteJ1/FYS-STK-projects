...
All of the functions $f, g, h$ from Eq. \eqref{init_cond} and Eq. \eqref{bound_cond}, can be functions dependent on x or constants, conditional to the system described by the PDE. 
...
In the diffusion equation introduced in section \ref{sec:diffeq}, we approximate the two partial differentiation terms as described in Eq. \eqref{eq:fd_dt} and Eq. \eqref{eq:fd_dx2}.
The approximation in Eq. \eqref{eq:fd_dt} has error term of order $\mathcal{O}(\Delta t)$, while Eq. \eqref{eq:fd_dx2} has error term of order $\mathcal{O}(\Delta x^2)$.
The derivation of both approximations from Taylor expansions of the function $u(x, t)$ around $(x_j, t_n)$ are included in Appendix \gaute{ref here}\ref{}.
The derivation of equation 
\begin{equation}
    \label{eq:diff_scheme}
    \frac{u_j^{n+1}-u_j^n}{\Delta t} = \frac{u_{j+1}^n - 2u_j^n + u_{j-1}^n}{\Delta x^2}
\end{equation}
\gaute{finish sentence}
Equation \ref{eq:diff_scheme} describes the entire finite difference scheme for the PDE defined in Eq. \eqref{eq:diffu}.
...
The schemes "step" through time, by solving Eq. \eqref{eq:diff_scheme} with respect to $u_j^{n+1}$.
\begin{figure}[h]
        \centering
        \begin{tikzpicture}
        ...
        \caption{The stencil of the scheme defined by Eq. \eqref{eq:diff_scheme}.} 
...
% \end{equation}
% Eq. \eqref{glorot} shows the normalized uniform distribution from which the weights are selected in Glorot, and Eq. \eqref{he} the normalized normal distribution from which values are selected in He. In both equations $W_{i,j}$ denote the weights between the $i$-th and $j$-th layers, and $m$ and $n$ the size of the $i$-th and $j$-th layer respectively. Although the methods were proposed as given above in their respective papers, it is not uncommon to implement Glorot as a normal distribution, and He as a uniform distribution. As of now there exists no common consensus of which versions are better, neither theoretically or empirically, and thus the choice is for each specific case.
...
%     C(\theta_t - \eta \nabla_\theta C(\theta_t))
% \end{equation}
% Another approach is calculating the function in Eq. \eqref{eq:line_search} for different values of $\eta$, and choosing the $\eta$ resulting in the smallest value. This technique is called \textit{line search} \citep[p. 84]{Goodfellow-et-al-2016}.
...
To address these issues, a series of algorithmic variations have been proposed to replace the step of updating the model (Eq. \eqref{eq:updt_std}).
...
% \end{split}
% \end{equation}
% The update-step of AdaGrad is displayed in Eq. \eqref{eq:updt_ada}, where $g_t^2$ is $g_t$ squared element-wise.
% The $\delta$ in Eq. \eqref{eq:updt_ada} is a small constant added for numerical stability.
...
% \end{split}
% \end{equation}
% RMSProp is implemented by the algorithm described in Eq. \eqref{eq:updt_rms}.
...
The update step in Eq. \eqref{eq:updt_std} is then replaced by
\begin{equation}
...
% This method replaces the update-step of Eq. \eqref{eq:updt_std} with Eq. \eqref{eq:updt_adam}.
...
The heat equation (Eq. \eqref{eq:diffu}) is a PDE governed by a number of physical laws. 
...
Using the notation from Eq. \eqref{u_tu_xx} we have $r = u_{xx} - u_t$. 
\begin{equation}
    C_1(x,t) = \frac{1}{n}\sum_{i=1}^{n}(u_{xx} - u_t)^2\mia{insert i}
\end{equation}

The initial conditions (Eq. \eqref{eq:initial_heat_eq}) make up the second cost function: 

\begin{equation}
    C_2(x,t) = \frac{1}{n}\sum_{i=1}^{n}(\hat{y_i}-sin(\pi x_i))^2
\end{equation}

Lastly, the boundary values should be zero as given in Eq. \eqref{eq:boundaries_heat_eq}. 
This can be expressed as the following cost function: 

\begin{equation}
    C_3(x,t) = \frac{1}{n}\sum_{i=1}^{n}\left( \hat{y}_i(x = 0)^2+\hat{y}_i(x = L)^2\right)
\end{equation}
