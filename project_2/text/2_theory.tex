...
The first version, as presented in Goodfellow \textit{et al.} (\citeyear[p. 291]{Goodfellow-et-al-2016}), is that we in each update of the model parameters, only use one randomly chosen \textit{mini-batch} of the training data to calculate the gradients.



The first view, as represented in Raschka, is that SGD

% Utilizing the same steps as with regular gradient descent - only now each step is only performed on a subset of data-points, being randomly selected from our complete set. In doing so, we're allowing for most of the accuracy of gradient descent to remain while improving the efficiency of the algorithm. 
% Stochastic gradient descent also allows for easier inclusion of new data-points, as they can simply be included as an additional step without the need of restarting the algorithm from scratch. 

% In its strictest definition, stochastic gradient descent only uses one data-point per step, but it is more common to pick a subset (also called a mini-batch) to get a more accurate tuning. 
%https://www.youtube.com/watch?v=vMh0zPT0tLI&ab_channel=StatQuestwithJoshStarmer 


\subsubsection{Learning rate}
\gaute{ - What is this thing???
- Tuning the learning rate: why a too high/low learning rate causes issues  - Varying learning rates }

\gaute{Learning rate schedulers}
...
\end{split}
\end{equation}
The update step of AdaGrad is displayed in Eq. \ref{eq:updt_ada}.
...
A general NN consists of an input layer, $k$ hidden layers and an output layer - all consisting of a varying number of nodes, as visualized in Fig. \ref{fig:NN}. 
The hidden layers are not hidden in the sense that they're not a visible part of the model; but rather that when using a trained model one never interacts with anything past the input and output layers, thus the internal nodes appear as hidden. 
...
The simplest model for an artificial neural network is called a \textit{feed forward neural network} (FFNN), where logically enough the flow of information moves only in one direction; forward. 
...
$n$ denotes the number of data points in the training data, and the prediction for the i-th data point is denoted $\Tilde{y}_i$. Comparing the expression above to Eq. \ref{cost_ols}, it's clear how the OLS regression method is inherently designed to minimize MSE. Given a loss-function of the squared distance between the prediction and the true value, the errors for cross-validation and bootstrapping (respectively Eq. \ref{cv_error} and Eq. \ref{bs_error}) will trivially be the equivalent of MSE. 
