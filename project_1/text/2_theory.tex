...
\subsubsection{Splitting data to avoid overfitting}
...
The error measure we truly want is the prediction error on an independent test data set: 
\begin{equation}\label{error_test}
    \text{Error}_{test} = ??
\end{equation}


However, when we use loss on the training data to quantify the performance of the model, we end up being optimistic about the error on a new independent set. 
...
K-fold cross-validation is such a method. The number K must be chosen by the developer. We divide our dataset into K parts, called folds. K-1 folds are used as training data, while the remaining fold is used as test data. The procedure is repeated K times, holding out a new fold each time. The estimated error is then the mean of the K test errors. \mia{likninger om error prediction}

Bootstrapping is a method where we draw with replacement from the original data to have a new data set for training. It should have the same size as the original data set. We generate many such bootstrap samples and repeat the training. \mia{more} The error is calculated as the mean of the error. \mia{likninger om error prediction}
...
An extension of the ordinary least squares method is to add a penalization term. A general term for this is shrinkage methods. There are many reasons for why this is often to prefer. \mia{cite for the last statement}

To perform OLS we assume that the matrix $\boldsymbol{X}^T\boldsymbol{X}$ is invertible \mia{ref to prev eq about this}. This may not always be the case due to correlation between the predictors in the data set or if $p > n$. Then the matrix will not be full rank. 
...
\mia{Generelt om dette}

\subsubsection{Ridge Regression}



\mia{Uttrykket for beta, tolkning, error estimation formel}
...
\mia{Fill out MSE and include R2}
...
$R^2$ is a measure of how much of the variance in the data the model explains. \mia{cite} The quantity is a number $\in [0,1]$. If $R^2 = 1$ the model perfectly explains all variance, whereas a value of 0 would mean does not explain any of the variance. The numerator is the sum of the squared residuals, also called RSS. The denominator is the total sum of squares, in short TSS. 
